...
public:
   ...
   friend std::ostream & operator<<( std::ostream & lhs, const rational & rhs ){
      return lhs 
         << "[" 
         << "0x" << std::hex << rhs.counter 
         << "/" 
         << "0x" << std::hex << rhs.denominator
         << "]";
   }   
   ...
   /// \brief
   /// Multiply a rational with another rational
   /// \details
   /// This operator*= multiplies a rational value with another rational value
   rational & operator*=( const rational & rhs ){
      counter *= rhs.counter;
      denominator *= rhs.denominator;
      reduce();
